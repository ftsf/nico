...
proc process(self: var Channel): float32 =
  ...
  of channelSynth:
    ...
    of synNoise2:
      if self.freq != 0.0:
        if self.nextClick <= 0:
          let lsb: uint = (self.lfsr2 and 1).uint
      ...
      if clock:
        ...
        if self.vibamount > 0:
          self.freq = self.basefreq +
                      sin(self.envPhase.float32 / self.vibspeed.float32) *
                      self.basefreq * 0.03 * self.vibamount.float32

        if self.pchange != 0:
          self.targetFreq = self.targetFreq +
                            self.targetFreq * self.pchange.float32 / 128.0
